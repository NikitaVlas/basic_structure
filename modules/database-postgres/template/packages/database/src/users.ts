...
export class UserRepository {
  ...
  async create(input: { emailNormalized: string; emailDisplay: string; passwordHash: string; role?: UserRole }) {
    const result = await this.database.query<UserRecord>(
      `INSERT INTO users (email_normalized, email_display, password_hash, role)
       VALUES ($1, $2, $3, $4)
       RETURNING id, email_normalized, email_display, password_hash, role, created_at`,
  ...
  async findByNormalizedEmail(emailNormalized: string) {
    const result = await this.database.query<UserRecord>(
      `SELECT id, email_normalized, email_display, password_hash, role, created_at
  ...
  async findPublicById(id: string) {
    const result = await this.database.query<Pick<UserRecord, "id" | "email_display" | "role" | "created_at">>(
      `SELECT id, email_display, role, created_at FROM users WHERE id = $1`,
      [id]
    );
    return result.rows[0] ?? null;
  }
