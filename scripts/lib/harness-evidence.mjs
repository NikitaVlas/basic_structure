...
export async function createHarnessEvidence(starterRoot,projectRoot,options={}){
  ...
  const evidence={schemaVersion:1,projectRoot:root,compliant:gate.compliant&&seo.status!=="fail",state:{schemaVersion:state.schemaVersion,starterVersion:state.starterVersion,extensionVersions:state.extensionVersions,extensionProvenance:state.extensionProvenance},catalogs:activeCatalogs.map(({activatedAt,...entry})=>entry),checks:gate.checks,seo};
