...
import { listCatalogExtensions } from "./catalog.mjs";
...
export async function inspectCatalogBundle(starterRoot, bundleRoot) {
  ...
  const files = (await walk(root)).filter((entry) => entry.path !== "catalog.json");
  ...
  return { root, publisher: metadata.publisher, id: metadata.id, version: metadata.version, cli: metadata.cli, files: files.length, bytes: files.reduce((sum, file) => sum + file.size, 0), identities: identities.sort(), digest: bundleHash.digest("hex") };
}
